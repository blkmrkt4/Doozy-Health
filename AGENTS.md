...
Operating guide for Codex on this project. Read this on every session, then read `PRD.md` end to end before answering questions about features, data model, or behaviour. The PRD is the source of truth for **what** to build; this file is **how** to build it and **what not to do**. If they disagree, stop and ask.
...
4. **The OpenRouter key comes from the host env first, `system_secrets` second.** Resolved by `getOpenRouterApiKey()`: the host environment variable (`OPENROUTER_API_KEY`, preferred; also accepts `OPEN_ROUTER` / `OPENROUTER_BOOTSTRAP_KEY`) wins, with the encrypted `system_secrets` row (the `/admin` field) as a fallback. This is deliberate for hosted deploys (e.g. Vercel) — the env var is durable across deploys and needs no decrypt round-trip, so a stale/undecryptable DB row can't strand the app (PRD §14.3). `SECRET_ENCRYPTION_KEY` and the env API key live in the host env / `.env.local`. **Other** secrets still live encrypted in `system_secrets`, read via `readSecret()` — don't add new env-read secrets without the same explicit sign-off.
...
**Drug-reference auto-backfill (PRD §5.7):** when a user adds a drug missing from the `drugs` cache, `lib/drug-reference.ts` may look up its *published population* PK values once via the `lookup_drug_pk` prompt and store the row for everyone. Allowed adjacent to rule #8 because the LLM only *looks up* reference values (fuzzy lookup) — the curve math stays deterministic TS — and every such row is stamped `reference_data.source = "llm_estimated"` so the chart labels it AI-looked-up / illustrative. PK reference values **only**; interactions stay curated (#9). A future admin/job can promote an `llm_estimated` row to curated.
