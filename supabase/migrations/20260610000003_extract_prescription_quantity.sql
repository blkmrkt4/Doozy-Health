-- Capture the quantity dispensed (e.g. "90 TAB" → 90) so we can later project a
-- run-out date. Forward-only: adds a new extract_prescription version with the
-- extra field and points the prompt at it; the prior version is kept.
...
- quantity_dispensed: the TOTAL number of units in the dispensed package, as a plain number, if shown (e.g. "90 TAB" → 90, "Qty: 30" → 30, "#60" → 60, "5 mL" → 5). This is the amount in the bottle/box, NOT the dose taken each time. Null if not stated.
...
Return ONLY the JSON object, no other text.$body$,
      ...
      'Adds quantity_dispensed (supply count) for run-out projection.');
