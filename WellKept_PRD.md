# WellKept — Product Requirements Document
...
- **Deterministic facts first (`lib/report/report-data.ts`).** All numbers are computed in plain TypeScript: per-medication scheduled-vs-logged dose counts, runs of consecutive misses (so a three-week gap reads differently from the occasional missed dose), a coarse consistency label, **doses logged above the prescribed amount** (compared via `convertDose`), per-field diary summary stats incl. **median** grouped general / by-medication (via `tracked_field_medications`) / labs (the `periodic` cadence) with **tracked substances** (alcohol/caffeine/nicotine) flagged, and a weekly timeline aligning dose counts with numeric-measure means. No number is ever produced by the LLM (hard rule #8).
- **Curated interactions (`facts.interactions`, hard rule #9).** The report surfaces drug/drug and drug/substance interactions among the in-scope set — active medications' canonical drugs **plus** substances the patient tracks in the diary, bridged to their `drugs` row (`lib/report/substances.ts`) — by querying the curated `drug_interactions` table only (`findInteractionsAmong`). The LLM never enumerates an interaction; it may only describe records already present in the facts. The curated catalogue (`lib/drug-catalogue.ts`) seeds common substance/OTC pairs (alcohol × SSRIs / benzodiazepines / opioids, NSAID × SSRI, etc.).
- **The report replicates the diary view.** Tracked measures render the same way as the diary: one combined 1–10 scale chart (`ScaleChart`) plus compact mini-cards (sparkline + avg/median/range) for everything else.
- **LLM narrates only (`lib/report/narrative.ts`, prompt `summarize_report_for_clinician`).** The model receives the pre-computed facts and writes observational prose into fixed JSON sections (overview, per-medication, adherence notes, diary observations, correlation observations, **interaction observations**, data caveats). It invents no numbers, computes no curve or dose, ranks no regimen, gives no advice (§6.1), and may describe **only** the interactions present in the facts. This is fuzzy summarisation, the allowed kind of LLM work.
...
- **Footer line on every screen and document:** *"WellKept is a wellness tool. It is not a medical device and does not provide medical advice. Consult your doctor."*
...
| **LLM gateway** | OpenRouter only, via `llmCall`. The OpenRouter key is resolved by `getOpenRouterApiKey()` — host env var (`OPENROUTER_API_KEY`) first, encrypted `system_secrets` row second (§14.3). Other secrets stay in `system_secrets`. | — |
...
- **User-perceived trust:** the user is willing to use only WellKept as their dose record without a backup spreadsheet or notes app — single-question in-app survey at 60 days.
...
6. **Admin backend foundation (§14).** `system_secrets` (encrypted via `SECRET_ENCRYPTION_KEY`), `system_settings` singleton, OpenRouter key, model catalog sync from `/models`, `llmCall(promptSlug, vars, opts?)` with primary + 2 fallback chain and `llm_call_logs` writes. The OpenRouter key is read host-env-first (`OPENROUTER_API_KEY`), with the encrypted `system_secrets` row as a fallback (§14.3).
...
- **API keys.** OpenRouter key entry, write-only; the UI shows the `value_masked` preview (`sk-or-v…wxyz`), never the raw value. Stored as a `system_secrets` row, encrypted via `lib/crypto.ts`. **Resolution order at runtime (`getOpenRouterApiKey()`): the host env var `OPENROUTER_API_KEY` (preferred for hosted deploys such as Vercel — durable across deploys, no decrypt round-trip; also accepts `OPEN_ROUTER` / `OPENROUTER_BOOTSTRAP_KEY`) first, then the encrypted `system_secrets` row via `readSecret()` as a fallback/override.** Env wins so a stale or undecryptable DB row can never strand the app; `SECRET_ENCRYPTION_KEY` must be a stable host env var for the DB fallback to decrypt. Other keys (drug-DB feeds, Twilio, SES) stay in `system_secrets` only.
